...
const ACHIEVEMENTS = [
  ...
  { id: 'time_collector',   cat: 'COLLECTION', rarity: 'RARE',      name: '时空收藏家',   desc: '收集10种时空系精灵',          check: s => s.timeCaught >= 10,     reward: { title: '时空行者' } },
  { id: 'chaos_collector',  cat: 'COLLECTION', rarity: 'RARE',      name: '混沌收藏家',   desc: '收集10种混沌系精灵',          check: s => s.chaosCaught >= 10,    reward: { title: '混沌之主' } },
  { id: 'cosmic_collector', cat: 'COLLECTION', rarity: 'RARE',      name: '宇宙收藏家',   desc: '收集5种宇宙系精灵',           check: s => s.cosmicCaught >= 5,    reward: { title: '星际旅人' } },
  { id: 'sound_collector',  cat: 'COLLECTION', rarity: 'RARE',      name: '音波收藏家',   desc: '收集5种音波系精灵',           check: s => s.soundCaught >= 5,     reward: { title: '音乐大师' } },
  ...
  { id: 'full_house',      cat: 'SOCIAL', rarity: 'UNCOMMON',  name: '温馨满屋',     desc: '房子家具数量达到30件',        check: s => s.furnitureCount >= 30 },
  ...
  { id: 'secret_speed_clear', cat: 'SECRET', rarity: 'UNCOMMON',
    ...
    hint: '提示：速度就是一切——先手秒杀',
    check: s => s.oneRoundTrainerWin >= 1, hidden: true },
  ...
  { id: 'lycoris_combo_10',      cat: 'BATTLE', rarity: 'UNCOMMON', name: '协作高手',
    desc: '使用协作技10次', check: s => s.comboAttacks >= 10 },
  ...
  // ===== 活动中心 =====
  { id: 'arena_50',         cat: 'BATTLE',   rarity: 'UNCOMMON',  name: '竞技场勇者',   desc: '竞技场累计获胜50场',          check: s => s.arenaWins >= 50 },
  ...
  // ===== 新增隐藏成就 (趣味挑战) =====
  { id: 'secret_pacifist', cat: 'SECRET', rarity: 'LEGENDARY', name: '和平主义者',
    desc: '不攻击任何一次，靠中毒/灼伤/缠绕等间接伤害赢得训练家战',
    hint: '提示：真正的强者不需要亲自动手...',
    check: s => s.pacifistWins >= 1, reward: { title: '和平主义者' }, hidden: true },
  { id: 'secret_full_team_wipe', cat: 'SECRET', rarity: 'EPIC', name: '满血团灭',
    desc: '对手6只精灵全部满血时一回合内全部击败（双打AOE连锁）',
    hint: '提示：当毁灭降临时无人能逃...',
    check: s => s.fullTeamWipeOneRound >= 1, reward: { title: '灭世者' }, hidden: true },
  { id: 'secret_same_species_6', cat: 'SECRET', rarity: 'EPIC', name: '六胞胎',
    desc: '用6只相同种类的精灵通关道馆',
    hint: '提示：数量就是力量...相同的六个！',
    check: s => s.sameSpeciesGymWin >= 1, reward: { title: '执念训练家' }, hidden: true },
  { id: 'secret_level_1_win', cat: 'SECRET', rarity: 'LEGENDARY', name: 'Lv.1的奇迹',
    desc: '用Lv.1的精灵击败Lv.50以上的敌人',
    hint: '提示：等级不代表一切——策略才是王道',
    check: s => s.lv1MiracleWins >= 1, reward: { title: '奇迹创造者' }, hidden: true },
  { id: 'secret_all_status', cat: 'SECRET', rarity: 'EPIC', name: '状态全书',
    desc: '在同一场战斗中让敌方经历全部5种异常状态',
    hint: '提示：灼伤、中毒、麻痹、睡眠、冰冻...一个都不能少',
    check: s => s.allStatusInflicted >= 1, reward: { title: '状态大师' }, hidden: true },
  { id: 'secret_no_damage_league', cat: 'SECRET', rarity: 'LEGENDARY', name: '联赛无伤',
    desc: '不受任何伤害通关整个联赛四轮',
    hint: '提示：完美的策略让对手无法触碰你...',
    check: s => s.leaguePerfectRun >= 1, reward: { title: '无瑕冠军' }, hidden: true },
  { id: 'secret_berry_clutch', cat: 'SECRET', rarity: 'RARE', name: '果实救赎',
    desc: '精灵即将被击杀时靠树果/被动回血存活并反杀',
    hint: '提示：最后一刻的果实...改变了一切',
    check: s => s.berryClutchKills >= 3, reward: { title: '果实达人' }, hidden: true },
  { id: 'secret_revenge_sweep', cat: 'SECRET', rarity: 'EPIC', name: '复仇之刃',
    desc: '己方5只精灵倒下后，最后一只精灵清场获胜',
    hint: '提示：倒下的同伴注视着你...用胜利回应他们！',
    check: s => s.revengeSweeps >= 1, reward: { title: '复仇者' }, hidden: true },
  { id: 'secret_catch_full_hp', cat: 'SECRET', rarity: 'RARE', name: '强制捕获',
    desc: '在敌方精灵满血时成功捕捉（非大师球）',
    hint: '提示：不需要削弱...直接收服！',
    check: s => s.fullHpCatches >= 3, reward: { title: '暴力捕手' }, hidden: true },
  { id: 'secret_10_crits_row', cat: 'SECRET', rarity: 'LEGENDARY', name: '天命所归',
    desc: '连续10次攻击全部暴击',
    hint: '提示：命运女神对你微笑...',
    check: s => s.maxConsecutiveCrits >= 10, reward: { title: '天选之人' }, hidden: true },
  ...
  { id: 'secret_speedrun_gym', cat: 'SECRET', rarity: 'EPIC', name: '极速通关',
    ...
    check: s => s.speedrunFirstBadge >= 1, reward: { title: '极速训练家' }, hidden: true },
  { id: 'secret_fossil_team', cat: 'SECRET', rarity: 'EPIC', name: '远古军团',
    desc: '队伍中6只精灵全部为岩石属性',
    hint: '提示：岩石之力——来自远古的呼唤',
    check: s => s.fullRockTeam >= 1, reward: { title: '化石猎人' }, hidden: true },
  { id: 'secret_dodge_master', cat: 'SECRET', rarity: 'RARE', name: '闪避之神',
    desc: '单场战斗中闪避敌方攻击5次以上',
    hint: '提示：打不中就是无敌...',
    check: s => s.maxDodgesInBattle >= 5, reward: { title: '闪避之神' }, hidden: true },
  { id: 'secret_world_tour', cat: 'SECRET', rarity: 'LEGENDARY', name: '世界旅行家',
    desc: '在每张地图上都捕捉至少5只不同精灵',
    hint: '提示：真正的旅行家会在每一站留下足迹...',
    check: s => s.worldTourComplete >= 1, reward: { title: '世界旅行家' }, hidden: true },
];
...
export const DEFAULT_ACH_STATS = {
  ...
  pacifistWins: 0,
  fullTeamWipeOneRound: 0,
  sameSpeciesGymWin: 0,
  lv1MiracleWins: 0,
  allStatusInflicted: 0,
  leaguePerfectRun: 0,
  berryClutchKills: 0,
  revengeSweeps: 0,
  fullHpCatches: 0,
  ...
  sanctuaryUpgrades: 0,
  relicsCollected: 0,
  maxResonanceCombos: 0,
  ultimateResonance: 0,
  guardianScoreTotal: 0,
  ecoGuardianEncounters: 0,
  gangKwLinkWins: 0,

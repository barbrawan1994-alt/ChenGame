...
export const INFINITY_FLOOR_MODIFIERS = [
  ...
  { id: 'focus', name: '集中', icon: '🎯', desc: '敌方暴击+10%', enemyStatMult: { crit: 10 } },
...
/** 里程碑层：通关后额外宝箱（不含 10/25/50/75/100 等特殊层） */
...
export function getInfinityMilestoneReward(floor) {
  ...
  if (floor >= 85) return { gold, item: 'master_ball', itemCount: 1, label: '传说补给箱' };
  if (floor >= 55) return { gold, item: 'hyper_potion', itemCount: 5, label: '高级补给箱' };
  if (floor >= 35) return { gold, item: 'great_ball', itemCount: 8, label: '精英补给箱' };

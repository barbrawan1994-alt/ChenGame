...
import { ANCIENT_BATTLEFIELDS, FRUIT_SEA_ZONES, JUTSU_SECRET_REALMS, SECT_SECRET_REALMS, KINGDOM_PVE_TASKS, KINGDOM_POSITIONS, PLAYER_STYLES, GENERAL_PVE_TACTICS, getActiveNationalCalamities, getCalamityWeekKey, getUnlockedFusionSystems, getUnlockedCrossSystems, calcCrossSystemPveBonuses } from '../../data';
...
const rewards=r=>[r?.gold && `${r.gold} 金币`,r?.item && '道具奖励',r?.jutsuMastery && `忍术熟练 +${r.jutsuMastery}`,r?.title && `称号「${r.title}」`,r?.kwContrib && `国战贡献 +${r.kwContrib}`].filter(Boolean).join(' · ');
const bonusRows=b=>[
...
export default function FusionHubScreen({badges,completeKingdomPveTask,fusionHubOpen,fusionHubTab,fusionState,getLocalDateStr,participateCalamity,party,selectGeneralTactic,selectKwPosition,selectPlayerStyle,setFusionHubOpen,setFusionHubTab,startFusionDungeon,onTactics,kingdomWar,ecoCrisisState,sanctuaryState,gold}) {
  ...
  const bonuses=calcCrossSystemPveBonuses({playerStyle:fusionState.playerStyle || {},kwPosition:fusionState.kwPosition});
  ...
  return <div className="joint-overlay" role="dialog" aria-modal="true" aria-label="联合冒险"><section className="joint-adventure">
    ...
    <main>
      {section==='routes' && <><div className="joint-filters">{ROUTES.map(([id,name])=><button key={id} aria-pressed={route[0]===id} onClick={()=>setFusionHubTab(id)}>{name}</button>)}</div><div className="joint-list">{route[2].map(def=>{
        const done=(fusionState[route[3]] || []).includes(def.id),locked=badges.length<def.reqBadges;
        const progress=fusionState.dungeonProgress?.[`${route[0]}_${def.id}`] || 0;
        return <article key={def.id}><div><span className="joint-condition">{done ? '已完成' : locked ? `需要 ${def.reqBadges} 枚徽章` : '可挑战'} · {def.steps?.length || 1} 阶段</span><h2>{def.name}</h2><p>{def.summary}</p><small>{rewards(def.reward)}</small><details><summary>阶段与目标</summary><ol>{(def.steps || []).map((step,i)=><li key={i}>{step.title || step.enemyName || {battle:'守卫战',boss:'首领战',puzzle:'机关解谜',explore:'区域探索',soothe:'安抚'}[step.type]}{i<progress && ' · 已完成'}</li>)}</ol>{def.sectIds?.length>0 && <p>需要对应门派伙伴随行。</p>}</details></div><button disabled={done || locked} onClick={()=>startFusionDungeon(route[0],def.id)}>{done ? <Check size={17}/> : <Swords size={17}/>} {done ? '已完成' : progress ? '继续挑战' : '开始挑战'}</button></article>;
      })}</div></>}
      {section==='calamity' && (unlocked.includes('national_calamity') ? <div className="joint-list">{active.length ? active.map(cal=>{const done=fusionState.calamitiesParticipated?.includes(`${cal.id}_${week}`);return <article key={cal.id}><div><span className="joint-condition">本周救援 · {cal.reqBadges} 枚徽章</span><h2>{cal.name}</h2><p>{cal.summary}</p></div><button disabled={done || badges.length<cal.reqBadges} onClick={()=>participateCalamity(cal.id)}>{done ? '本周已参与' : '参与救援'}</button></article>;}) : <p>本周暂无可参与的灵灾。</p>}</div> : <p className="joint-locked">8 枚徽章开放灵灾救援。</p>)}

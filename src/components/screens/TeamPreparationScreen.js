...
import { POKEDEX, TYPES, ACCESSORY_DB, SECT_DB } from '../../data';
...
export default function TeamPreparationScreen({party,narutoState,fusionState,ultraState,badges,onPrepare,onBreathing,onBack,onUltra,onGuide,onInspect,onEquip,onReorder,onRecall,onTraining,renderAvatar,getMoveCategory}) {
  ...
  const [selectedTab,setTab]=useState('moves');
  const eligible=getEligiblePreparedJutsu(pet,narutoState);
  const tab=selectedTab==='jutsu' && !eligible.length ? 'moves' : selectedTab;
  ...
  const host=party.find(unit=>unit.uid===ultraState?.hostUid);
  const equippedDevice=host && isUltraUnlocked(ultraState,ultraState.heroId) ? getUltraDevice(ultraState.heroId) : null;
  const device=host?.uid===pet?.uid ? equippedDevice : null;
  const equipment=[0,1].map(slot=>typeof pet?.equips?.[slot]==='string' ? ACCESSORY_DB.find(item=>item.id===pet.equips[slot]) : pet?.equips?.[slot]);
  return <section className="screen team-preparation" aria-label="队伍战术室">
    ...
    <div className="team-layout">
      ...
      <main className="team-config">
        ...
        {tab==='moves' && <><div className="team-section-heading"><h2>当前技能</h2><span>{pet.moves?.length || 0} / 4</span></div><div className="team-moves">{(pet.moves || []).map((move,i)=><div key={`${move.id || move.name}:${i}`}><span className="team-move-index">{i+1}</span><div><strong>{move.name}</strong><small>{TYPES[move.t]?.name || move.t} · {move.p ? `${getMoveCategory(move.t,move)==='physical' ? '物理' : '特殊'}威力 ${move.p}` : '变化技能'} · PP {move.pp} / {move.maxPP || move.pp}</small></div></div>)}</div>{tactic && <div className="team-role"><Shield size={19}/><p>{tactic.tip}</p></div>}{recall.length>0 && <details className="team-recall"><summary>回忆已学技能 <span>{recall.length}</span></summary>{recall.map(move=><div key={move.id}><div><strong>{move.name}</strong><small>{TYPES[move.t]?.name} · {move.p ? `威力 ${move.p}` : '变化技能'}</small></div><button disabled={!!pet.pendingLearnMove} onClick={()=>onRecall(pet.uid,move.id)}>回忆<ChevronRight size={14}/></button></div>)}<p>回忆消耗 300 金币；四个技能已满时需替换一个。</p></details>}</>}
        {tab==='equipment' && <><div className="team-section-heading"><h2>携带装备</h2><span>2 个位置</span></div><div className="team-equipment">{equipment.map((item,slot)=><button key={slot} onClick={()=>onEquip(index,slot)}><Shield size={21}/><span><strong>{item?.displayName || item?.name || `装备位置 ${slot+1}`}</strong><small>{item?.extraSkill ? `附带技能：${item.extraSkill.name}` : item?.desc || '选择装备'}</small></span><ChevronRight size={16}/></button>)}</div><dl className="team-capabilities"><div><dt>恶魔果实</dt><dd>{pet.devilFruit?.name || '未食用'}</dd></div><div><dt>尾兽</dt><dd>{pet.bijuu?.name || '未封印'}</dd></div><div><dt>门派</dt><dd>{pet.sectId ? `${SECT_DB[pet.sectId]?.name || '已入门'} · 修行 ${pet.sectLevel || 0} 层` : '未加入'}</dd></div><div><dt>变身器</dt><dd>{device?.name || '未携带'}</dd></div></dl></>}

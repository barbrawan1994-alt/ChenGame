                ...
                <div className="battle-move-pane">
                    ...
                    {/* 独立操作区，不参与技能网格的高度分配。 */}
                        ...
                        <div className="battle-action-list" aria-label="其他行动">
                            ...
                            {battle.type === 'ultra_trial' && <button type="button" className="action-btn-h" onClick={handleDefeat} title={battle._kaijuRaid ? '撤退会损失出征押金' : ''}><LogOut size={16} />{battle._kaijuRaid ? '撤退讨伐' : '结束试炼'}</button>}
                            ...
                            {(() => { const cp = isDoubleBattle ? (doubleCurrentPet || p) : p; if (!cp.ultraHeroId) return null; const reason = getUltraTransformBlock(battle, cp); return <button type="button" className="action-btn-h" style={{background:cp.ultraTransformed ? '#286657' : '#91394c'}} disabled={!!reason} title={reason || `使用${getUltraDevice(cp.ultraHeroId)?.name} · ${ULTRA_BY_ID[cp.ultraHeroId]?.name} · 三回合光能，必杀后解除`} onClick={executeUltraTransform}><Sparkles size={15} />{cp.ultraTransformed ? `光能 ${cp.ultraTurnsLeft}` : battle.ultraUsed ? '本场已用' : '使用变身器'}</button>; })()}
                            {!isDoubleBattle && (() => {

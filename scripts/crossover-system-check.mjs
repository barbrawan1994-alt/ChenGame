...
import {
  ...
  getQueuedDoubleMoveResourceCost,
  getRegeneratedBattleResource,
  getUnlockedBattleResourceMax,
  isSelfTargetingCombatMove,
  resolveDomainActivation,
  spendMovePP,
} from '../src/utils/combatRules.js';
...
check(getEffectiveChakraCost({ chakraCost: 40 }, { chakraCostMult: 0.85 }) === 34, '查门共鸣15%折扣统一按实际34点查克拉结算');
check(getEffectiveChakraCost({ chakraCost: 1 }, { chakraCostMult: 0.1 }) === 1, '查克拉折扣不会把有消耗的忍术降为零费');

const dragonDomain = { ownerSide: 'player', turnsLeft: 3, effect: DOMAINS.DRAGON.effect };
check(Math.abs(getDomainDamageMultiplier(dragonDomain, 'player', 'enemy') - 2.21) < 1e-9, '龙领域攻击+70%和敌防-30%都会正确提高领域方伤害');
const waterDomain = { ownerSide: 'enemy', turnsLeft: 3, effect: DOMAINS.WATER.effect };
check(Math.abs(getDomainDamageMultiplier(waterDomain, 'player', 'enemy') - (1 / 1.5)) < 1e-9, '防御型领域会正确降低对领域方造成的伤害');

const playerDomain = { name: '玩家领域', ownerSide: 'player', turnsLeft: 2, effect: {} };
const enemyClash = resolveDomainActivation(playerDomain, 'enemy', DOMAINS.FIRE, 'FIRE');
check(enemyClash.isClash && enemyClash.activeDomain === null && enemyClash.opposingDomainName === '玩家领域', '敌方可主动对撞并抵消玩家领域');
const enemyDomain = { name: '敌方领域', ownerSide: 'enemy', turnsLeft: 2, effect: {} };
const playerClash = resolveDomainActivation(enemyDomain, 'player', DOMAINS.WATER, 'WATER');
check(playerClash.isClash && playerClash.activeDomain === null, '玩家与敌方领域对撞遵循同一规则');

const cursedMove = { name: '赫', pp: 2, maxPP: 2 };
check(spendMovePP(cursedMove).ok && cursedMove.pp === 1, '咒术施放会消耗PP');
check(spendMovePP(cursedMove).ok && cursedMove.pp === 0 && !spendMovePP(cursedMove).ok, '咒术PP耗尽后不可继续施放');
check(getUnlockedBattleResourceMax(115, false) === 0 && getUnlockedBattleResourceMax(115, true) === 115, '未解锁忍术/咒术的单位不会获得空置战斗资源');
check(getRegeneratedBattleResource(51, 460, 15) === 66, '双打回合恢复从两次行动后的共享余额结算，不会被首位精灵旧值覆盖');

const queuedBattle = {
  phase: 'double_input_2',
  doubleActions: [{ activeIdx: 0, moveIdx: 0 }],
  playerCombatStates: [{ combatMoves: [{ isJutsu: true, chakraCost: 40 }] }],
  _resonanceFx: { chakraCostMult: 0.85 },
};
check(getQueuedDoubleMoveResourceCost(queuedBattle, 'chakra') === 34, '双打第二行动会预留第一行动的折后查克拉');
check(isSelfTargetingCombatMove({ p: 0, effect: { type: 'PROTECT' } }), '双打保护技能无需选择敌方目标');
check(isSelfTargetingCombatMove({ p: 0, effect: { healPercent: 0.3 } }), '双打自我治疗技能无需选择敌方目标');
check(!isSelfTargetingCombatMove({ p: 0, effect: { type: 'DEBUFF', target: 'enemy' } }), '双打减益技能仍要求选择敌方目标');

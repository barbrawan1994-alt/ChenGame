...
const raceBalanceData = loadProjectModule('src/data/raceBalance.js');
...
check('竞速使用权威金币和每日次数结算，平速排序保持传递性', () => {
  ...
  assert.ok(branch.includes('resolveRaceContest({'));

  const makeRandom = values => {
    let index = 0;
    return () => values[index++] ?? 0.5;
  };
  const rivalWinsTie = raceBalanceData.resolveRaceContest({
    player: { name: 'player', baseSpeed: 100 },
    partySpeeds: [100],
    rivals: [{ name: 'rival', baseSpeed: 100 }],
    random: makeRandom([0.5, 0.8, 1 / 11, 0.2]),
  });
  const playerWinsTie = raceBalanceData.resolveRaceContest({
    player: { name: 'player', baseSpeed: 100 },
    partySpeeds: [100],
    rivals: [{ name: 'rival', baseSpeed: 100 }],
    random: makeRandom([0.5, 0.1, 1 / 11, 0.9]),
  });
  assert.equal(rivalWinsTie.runners[0].name, 'rival');
  assert.equal(playerWinsTie.runners[0].name, 'player');
});
...
check('火影日试炼进度可安全持久化并拒绝过期或损坏状态', () => {
  const valid = narutoData.normalizeNarutoExamProgress({
    phase: 'forest', startedDate: '2026-07-12', survivalWave: 2, survivalWaves: 3,
    score: 120, forestProgress: 8, scrolls: { heaven: 4, earth: -1 }, finalsRound: 1,
    difficulty: { waves: 3, enemyPerWave: [2, 3, 99], lvMod: 4, finalsExtra: 2 },
  });
  assert.equal(valid.phase, 'forest');
  assert.equal(valid.startedDate, '2026-07-12');
  assert.equal(valid.scrolls.heaven, 1);
  assert.equal(valid.scrolls.earth, 0);
...
check('训练家战术室按徽章渐进解锁且跨体系叠加不突破硬上限', () => {
  const early = systemFusionData.getUnlockedFusionSystems(0);
  const endgame = systemFusionData.getUnlockedFusionSystems(10);
  assert.ok(early.includes('pet'));
  assert.equal(early.includes('fusion_hub'), false);
  assert.ok(endgame.includes('fusion_hub'));
  const bonus = systemFusionData.calcFusionPveBonuses({
    sectId: 2,
    generalTacticId: 'zhuge_liang',
    jutsuSynergy: { purifyBonus: 99, protectBonus: 1, captureBonus: 1, escapeTurnReduce: 9 },
  });
  assert.equal(bonus.purifyBonus, 25);
  assert.ok(bonus.protectBonus <= 0.35);
  assert.ok(bonus.captureBonus <= 0.18);
  assert.ok(bonus.escapeTurnReduce <= 2);
});
...
check('旧存档派遣状态按权威槽位、UID、时长和分支奖励归一化', () => {
  ...
  const activity = saveNormalizer.normalizeActivitySaveState({
    ...
    raceState: { dailyRaces: 99, totalWins: -1, lastCandyDate: '2026-07-11' },
  }, now);
  ...
  assert.equal(activity.raceState.lastCandyDate, '2026-07-11');
});
...
check('名将招募与活动报名在确认时复核名额、价格、冷却和金币', () => {
  ...
  assert.ok(activity.includes("if (!started)"));
  assert.ok(activity.indexOf('if (!started)') < activity.indexOf('goldRef.current -= actualFee'));
  assert.ok(activity.indexOf('if (!started)') < activity.indexOf('dungeonCooldownsRef.current = nextCooldowns'));
  assert.ok(activity.includes('报名费与次数均未消耗'));
});

check('华丽大赛最高奖励在五回合规则内可达但仍要求接近完美', () => {
  const tiers = battlesData.CONTEST_CONFIG.beauty.tiers;
  const topThreshold = tiers[0].min;
  const theoreticalCeiling = 205;
  assert.ok(topThreshold <= theoreticalCeiling);
  assert.ok(topThreshold >= 185);
  assert.ok(topThreshold > tiers[1].min);
});
...
check('活动中心入口不会再叠加全局消息弹窗遮挡活动卡片', () => {
  ...
  assert.equal(branch.includes('showMessage('), false);
});

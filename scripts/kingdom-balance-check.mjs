...
const { SANGUO_GENERALS } = generalsData;
const {
...
// 22. 名将录扩充必须五方等量、唯一，并完整迁移到国战政治状态。
{
  ...
  check(true, '名将录共 400 人、五方各 80 人，旧档迁移后每名将恰好进入一个国战阵营');
}
...
// 24. 外交必须有成本门槛、行动上限，并同时约束玩家与 AI 的攻击许可。
{
  ...
  check(true, '修好→缔约需要两次行动，盟约双向禁战；每日上限与反霸主联合条件均生效');
}
...
// 27. 静置玩家不享受个人战力加成，赛季胜者不能由初始领地顺序锁死。
{
  ...
  assert.ok(winnerFactions >= 3, `至少应有三方能夺冠，实际 ${JSON.stringify(winners)}`);
  assert.ok(maxWins < seeds * 0.65, `任何一方静置夺冠率不得超过 65%，实际 ${JSON.stringify(winners)}`);

...
async function main() {
  ...
  for (let i = 0; i < ULTRA_HEROES.length; i++) {
    ...
    if (!source) throw new Error(`Missing verified portrait source: ${hero.id}`);
    if (!fs.existsSync(file)) {
      const input = execFileSync('curl', [...proxyArgs, '-fsSL', '--retry', '2', '--max-time', '25', encodeURI(decodeURI(source))], { maxBuffer: 12 * 1024 * 1024 });
      await sharp(input).resize(320, 320, { fit: 'contain', background: '#e0e7e3' }).webp({ quality: 85 }).toFile(file);
    }
    manifest.push({ ...sourceManifest.manifest.find(item => item.id === hero.id), id: hero.id, source, bytes: fs.statSync(file).size });
  ...
  fs.writeFileSync(path.join(destination, 'sources.json'), JSON.stringify({ checkedAt: new Date().toISOString().slice(0, 10), manifest }, null, 2) + '\n');
}
